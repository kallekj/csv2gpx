...
from csv2gpx.core import export_filename, export_gpx, parse_csv_log, safe_float
...
def test_parse_csv_log_reads_included_sample() -> None:
    ...
    assert log.points[0].values["sog_knots"] == 5.19006
...
def test_export_gpx_limits_rows_and_preserves_utc_time() -> None:
    ...
    content = export_gpx(log, start, end)
    ...
    sog = extensions.find("g:sog_knots", ns)
    assert sog is not None
    assert sog.text == "5.40389"
    assert extensions.find("g:oil_temp", ns) is None
